...
#2.Normalizacion global
todas_las_features = np.array([item['mel_spectrogram'] for item in train_data])
mean_global = todas_las_features.mean()
std_global  = todas_las_features.std()
print(f"Media global: {mean_global:.4f}")
print(f"Std global:   {std_global:.4f}")
# 3. Creación de los Datasets (añadir device_to_label)
# Sin esto, el Dataset no sabrá que Huawei es 0, Poco es 1, etc.
train_dataset = ParesAudioSplicingDataset(train_data, device_to_label, mean_global, std_global)
val_dataset   = ParesAudioSplicingDataset(val_data,   device_to_label, mean_global, std_global)




# 4. DataLoaders listos para el Trainer
...
ENTRENAR=True  # ← pon True la PRIMERA vez para generar el archivo
...
train_emb_loader = DataLoader(EmbeddingExtractionDataset(train_data, device_to_label, mean_global, std_global), batch_size=32)
val_emb_loader   = DataLoader(EmbeddingExtractionDataset(val_data,   device_to_label, mean_global, std_global), batch_size=32)
